...
var WebAudioFontPlayer = /** @class */ (function () {
    ...
    WebAudioFontPlayer.prototype.queueWaveTable = function (audioContext, target, preset, when, pitch, duration, volume, slides) {
        ...
        // 🆕 VALIDAÇÃO DEFENSIVA: verificar preset antes de buscar zone
        if (!preset) {
            console.warn('queueWaveTable: preset não fornecido');
            return null;
        }
        
        if (!preset.zones || !Array.isArray(preset.zones) || preset.zones.length === 0) {
            console.warn('queueWaveTable: preset sem zones válidas');
            return null;
        }
    ...
    WebAudioFontPlayer.prototype.findZone = function (audioContext, preset, pitch) {
        ...
        if (!preset) {
            console.warn('findZone: preset não fornecido');
            return null;
        }
        
        if (!preset.zones || !Array.isArray(preset.zones)) {
            console.warn('findZone: preset.zones inválido', preset);
            return null;
        }
        
        if (preset.zones.length === 0) {
            console.warn('findZone: preset sem zones', preset);
            return null;
        }
        
        // Procurar zone apropriada para o pitch
        for (var i = preset.zones.length - 1; i >= 0; i--) {
            zone = preset.zones[i];
            
            // 🆕 Verificar se zone é válida
            if (!zone) {
                continue;
            }
            
            // Verificar range de notas
            var lowRange = zone.keyRangeLow !== undefined ? zone.keyRangeLow : 0;
            var highRange = zone.keyRangeHigh !== undefined ? zone.keyRangeHigh : 127;
            
            if (lowRange <= pitch && highRange + 1 >= pitch) {
                // 🆕 Verificar se zone tem buffer ou dados para decodificar
                if (!zone.buffer && !zone.sample && !zone.file) {
                    console.warn('findZone: zone encontrada mas sem buffer/sample/file', zone);
                    continue;
                }
                break;
            }
        ...
        // Se não encontrou zone válida, tentar usar a primeira disponível
        if (!zone || (!zone.buffer && !zone.sample && !zone.file)) {
            for (var j = 0; j < preset.zones.length; j++) {
                if (preset.zones[j] && (preset.zones[j].buffer || preset.zones[j].sample || preset.zones[j].file)) {
                    zone = preset.zones[j];
                    console.log(`findZone: usando zone alternativa [${j}] para pitch ${pitch}`);
                    break;
                }
            }
        }
        ...
        try {
            ...
            console.log('findZone: erro ao ajustar zone', ex);
        }
